...
--
-- Companion code for §5 of "Integrating AI into a Live BI Platform —
-- The Patterns That Actually Held Up". See the article for context.
--
-- Three tables: sessions, turns, tool_calls. The turn-level table is the
-- workhorse referenced in the article. A minimal sessions table is
-- included (the turns FK needs it, and the file should run standalone);
-- the tool_calls table is sketched at the bottom. All three follow the
-- same convention — structured columns for the things we'd query, JSONB
-- for the things we wouldn't, FK back to session_id / turn_id so a
...
-- ---------------------------------------------------------------------
-- Sessions: one row per user session. Minimal version — production adds
-- cumulative token/turn counters and a JSONB summary of all turns for
-- fast no-join session reload.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Tool calls: one row per MCP tool execution within a turn.
--
-- Same convention as the others: structured columns for queryable fields,
-- JSONB for the open-ended ones, FK back to (session_id, turn_id) so a turn
-- can be reconstructed call-by-call. The FK is valid because the turns table
-- carries UNIQUE (session_id, turn_id).
-- ---------------------------------------------------------------------
...
CREATE TABLE fw_genai_logs.fw_genai_logs_tool_calls (
    ...
    sequence      INTEGER,                 -- call order within the turn
...
CREATE INDEX IF NOT EXISTS ix_fgl_tool_calls_turn
    ON fw_genai_logs.fw_genai_logs_tool_calls (session_id, turn_id, sequence);
