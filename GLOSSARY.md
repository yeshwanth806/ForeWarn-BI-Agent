...
A plain-English companion to **["Designing for Determinism: GenAI Inside a Mature BI-Analytics Platform"](../architecture-blog-v13.md)**. If you bumped into a term in the post and weren't sure what it meant, look it up here. Definitions are intentionally short and informal — not textbook-precise.
...
## Greenfield project
A project started from a blank slate, with no existing code, contracts, or constraints to work around. The article notes that this was *not* a greenfield project — the platform already existed and dictated many of the rules.
...
## Fallback Chain
An ordered list of LLM providers to try if the first one fails. Try Gemini → if it errors → try GPT → if it errors → try Claude. The user never sees the failure unless every option in the chain fails.
...
A separate, isolated program spawned by the main app to do one job. If it crashes, leaks memory, or hangs, the main app keeps running — the damage is contained. We spawn a fresh subprocess for every tool call.
...
## ReAct agent
An LLM "agent" pattern that alternates between **Reasoning** ("I should look up the file first") and **Acting** ("call the file_summary tool"). LangChain's `create_react_agent` builds this loop for you. It's how the LLM uses tools step by step instead of answering in one shot.
...
## Subquery
A SELECT statement nested inside another SELECT — a query within a query. They look like parenthesized SQL fragments. The validator has to know whether parentheses contain a subquery (real SQL) or just function arguments.
...
Python's way of doing many things at once without using multiple threads. The "event loop" is what schedules and runs all the in-flight tasks. `asyncio.to_thread` is a way to run a slow synchronous task without blocking the loop.
